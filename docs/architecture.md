...
**Heatmap card DOM shape (v1.2 change, legend moved in v1.3).** `#heatmap-card` in `history.html` is no longer a single element that `renderHeatmap()` clears and rebuilds wholesale. It now has two children: a static `.heatmap__header` and a `#heatmap-content` div. `history.js` passes `#heatmap-content` to `renderHeatmap()`, and still toggles `#heatmap-card`'s own `hidden` attribute to show/hide the whole card (header + content together) exactly as before — the CSV button is hidden/shown along with the heatmap rather than independently, which is an intentional side effect (design.md §4.1/§4.2): it no longer appears before the first page of data has loaded.

`.heatmap__header` holds `#heatmap-legend` and `#csv-button`, inline, legend on the left. **v1.3:** the legend moved here from inside `#heatmap-content` — `ui.js` now exports `renderHeatmapLegend(container)` as a separate function from `renderHeatmap(container, opts)`, since the legend never depends on data. `history.js` calls `renderHeatmapLegend(heatmapLegendEl)` exactly once, at module load (not on every `renderHeatmapCard()` call the way `renderHeatmap()` is) — it's stateless and idempotent, so calling it more than once would be harmless, just unnecessary. `#heatmap-content` still holds everything data-dependent that `renderHeatmap()` clears and rebuilds on every call: the weekday row, the weeks grid, and the expanded-day panel. `#csv-button` remains the one node neither function ever touches except through `history.js`'s own `renderCsvButton()` — its identity and click listener must survive every re-render, exactly as before.
