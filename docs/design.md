...
│ Feed history                        │  h1
│                                     │
│ ┌──[0·1·2·3·4+ legend]──────[⬇]──┐ │  §4.1/§4.2 (v1.3: legend + icon-only
│ │ [ heatmap: 5 weeks, Sun...Sat ] │ │  CSV button share one header row,
│ └─────────────────────────────────┘ │  legend left, CSV button right)
│                                     │
...
- A `--surface` card, full column width. **v1.2:** the card gained a thin header row at the very top — a static header the heatmap's own re-render never clears (architecture.md §4 flags exactly which DOM node `renderHeatmap` is allowed to wipe, since it's no longer the whole card). **v1.3:** that header row now holds the legend (moved up from below it) on the left and the icon-only CSV button (§4.2) on the right, in one inline row — Dathan's ask, both his "move the CSV button down and in-line with the legend" request and the button's icon-only restyle. The legend itself (4 swatches + "0" / "1" / "2" / "3" / "4+", left to right, muted 13px text beside the swatches) is unchanged in content from v1.1, just relocated; `renderHeatmapLegend()` builds it once (it never depends on data) rather than on every heatmap re-render. Below the header: a 7-column grid, unchanged from v1.1: `Sun Mon Tue Wed Thu Fri Sat` as a muted 12px header row, then `HEATMAP_WEEKS` (5) rows of date cells, oldest week first (top), current (possibly partial) week last (bottom) — so it reads top-to-bottom like a wall calendar, newest at the bottom nearest the list it introduces.
...
### 4.2 Download CSV button (v1.1; restyled and moved in v1.2; **icon-only and moved again in v1.3**)
- **v1.2 change:** moved off its own full-width row under `<h1>` into the heatmap card's new header row (§4.1). Restyled from the outlined `.btn--outline` (visually heavy, competed with the heatmap for attention right above it) to the existing `.btn--text` variant (text-colored `--accent-strong`, no fill or border, `flex: none` so it doesn't stretch) — the app's established "subtle" button style, not a new one.
- **v1.3 change:** the visible "CSV" label is gone entirely — icon only (`.btn--icon-only`, a fixed 44px-wide `.btn--text` variant) — and the button now sits inline with the legend (§4.1) at the right edge of the header row, instead of alone at the top-right. Dropping the text loses no information: the aria-label (below) already won over the visible text for the accessible-name computation, so it was always the button's real name even when "CSV" was visible.
- While fetching all pages for the export, the icon swaps to the existing spinner (`aria-hidden`) and the button disables — same `checking`/`loading` visual language as the Log button (§3.4). There is no visible "Preparing…" text any more (v1.3); the spinner and disabled state are the only busy-state signal, consistent with the button being icon-only throughout.
- `aria-label="Download {pet name}'s feed history as CSV"` (AC-19.5) — unchanged since v1.1, and now the button's *only* accessible name source (there's no visible text left to fall back to).
- Still a real `<button>`, still keyboard-reachable, still at least 44×44 (`.btn--icon-only` sets a fixed 44px width against `.btn`'s 44px min-height, so the compact visual icon never shrinks the actual tap target).
...
| CSV button (v1.3: icon-only, no visible text) | Download {pet}'s feed history as CSV (accessible name only) |
...
- **Screen-reader labels for icon-only elements.**
  ...
  - CSV button (v1.2, icon-only since v1.3): `aria-label="Download {pet name}'s feed history as CSV"` — unchanged wording from v1.1. Genuinely icon-only as of v1.3 (no visible text at all), so this is now its sole accessible name, not just the larger share of it.
...
| Zuumi / Banh Mi photos | **Supplied as of v1.2, re-cropped in v1.3** | Dathan supplied both; cropped square to face, resized to 176×176 (matching Pumo's own asset dimensions) and saved as `frontend/assets/zuumi.jpg` / `frontend/assets/banh-mi.jpg`, referenced by `pets.photo_url` (contract.md §1/§2). **v1.3:** the crop was pushed too far toward the top of the frame (nose near the bottom edge) — re-cropped so each pet's nose sits at roughly the vertical center of the square, matching Pumo's own photo's framing. | Same processing as Pumo's photo, framing corrected. All 3 pets now show a real photo; the placeholder below is exercised only by a hypothetical future 4th pet. |
